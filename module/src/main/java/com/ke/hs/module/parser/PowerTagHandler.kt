...
import com.ke.hs.module.entity.PowerTag
import com.ke.hs.module.entity.Zone
import com.ke.hs.module.entity.ZoneUpdatable
...
interface PowerTagHandler {
     ...
     * 处理一个事件
     */
    suspend fun handle(powerTag: PowerTag)
...
class PowerTagHandlerImpl @Inject constructor(
    ...
    init {
        GlobalScope.launch {
            allCard = getAllCardUseCase.execute()
        }
    }
...
//        MutableStateFlow<GameEvent?>(null)
    ...
    override val gameEventFlow: Flow<GameEvent?>
        get() = _gameEventFlow.receiveAsFlow()
...
//    var currentDeck: CurrentDeck? = null
    ...
    override suspend fun handle(powerTag: PowerTag) {
...
//                        handleOpponentHandChanged(it)
    ...
    private suspend fun onGraveyardCardsChanged(cardId: String?, user: Boolean) {
        if (cardId == null) {
            return
        }
        _gameEventFlow.send(GameEvent.InsertCardToGraveyard(cardId, user))
    }
...
//        allCard.find { it.id == cardId } ?: throw RuntimeException("id为 $cardId 没有这张牌")
    ...
    private suspend fun removeCardFromDeck(cardId: String) {
        _gameEventFlow.send(GameEvent.RemoveCardFromUserDeck(cardId))
    }

    private suspend fun insertCardToDeck(cardId: String) {
        _gameEventFlow.send(GameEvent.InsertCardToUserDeck(cardId))
    }


    private suspend fun handleTagChange(tagChange: PowerTag.PowerTaskList.TagChange) {
...
//        _deckLeftCardList.value = deckCardList
        ...
        _gameEventFlow.send(GameEvent.OnGameOver(game))
    }
...
//        "游戏开始了".log()
        ...
        currentTurn = 0
        _gameEventFlow.send(GameEvent.OnGameStart)
